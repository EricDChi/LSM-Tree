...
public:
    ...
    SStable(uint64_t SST_timeStamp, std::string path, std::vector<std::tuple<uint64_t, off64_t, uint32_t>> tuples);
    SStable(std::string fullPath);
    ~SStable();
    
    void put_vlog(std::string vlog, uint64_t key, uint32_t vlen, std::string &s, off64_t &offset);
    ...
    std::tuple<uint64_t, off64_t, uint32_t> get_tuple(uint64_t key);

    uint64_t get_min_key() {return min_key;}

    uint64_t get_max_key() {return max_key;}

    std::vector<std::tuple<uint64_t, off64_t, uint32_t>> get_tuples() {return tuples;}
